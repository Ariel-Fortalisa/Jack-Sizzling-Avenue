...
function itemtable(page, search) {
    ...
    $.ajax({
        url: "./ajax/fetch-archiveIngredients.php",
        type: 'get',
        content: "json",
        data: {
            request: 'items',
        ...
        success: function (result) {
            var table = $('#archiveIngrendients-table tbody');
            table.html('')
            var count = (page * 10) - 9;
                ...
                table.append(`
                    <tr>
                       ...
                       <td> 
                             ...
                             <button class="uil uil-history restore-btn" onclick="restoreItem(${item.id});modalShow('restoreIngredientModal')"></button>
            ...
            paginate($("#ArchiveIngredients_pagination"), page_count, page, 'itemtable');
        }
    })
                    ...
                    BatchTableModal.append(`
                       <tr> 
                            ...
                            <td class="text-center">${batchStatus}</td>
                                <td><button class="uil uil-edit edit-btn" onclick="setEditBatch(${batch.batch_id}, '${stock.item_name}',${batch.qty}, ${batch.cost}, '${batch.expiration_date}', ${batch.expiration_status});modalShow('editBatchModal')"></button>
                            <button class="uil uil-archive archive-btn" onclick="setArchiveBatch(${batch.batch_id}, ${batch.archive_status});modalShow('archiveBatchModal')"></button>                       
                        </td>
                        </tr>
...
function setEditBatch(batch_id, item_name, qty, cost, expiration_date) {
    //edit batch modal
...
function setArchiveBatch(id, status) {
    //archive batch
